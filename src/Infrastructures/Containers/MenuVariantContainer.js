...
} from "../../Applications/MenuVariants/UseCases/index.js";
...
export default function registerMenuVariantContainer({ container, overrides = {}, prisma }) {
	...
	const listMenuVariantsUsecase =
		overrides.listMenuVariantsUsecase ?? new ListMenuVariantsUsecase({ menuVariantService });
	const getMenuVariantUsecase = overrides.getMenuVariantUsecase ?? new GetMenuVariantUsecase({ menuVariantService });
	const createMenuVariantUsecase =
		overrides.createMenuVariantUsecase ?? new CreateMenuVariantUsecase({ menuVariantService });
	const updateMenuVariantUsecase =
		overrides.updateMenuVariantUsecase ?? new UpdateMenuVariantUsecase({ menuVariantService });
	const deleteMenuVariantUsecase =
		overrides.deleteMenuVariantUsecase ?? new DeleteMenuVariantUsecase({ menuVariantService });
	const menuVariantPresenter = overrides.menuVariantPresenter ?? new MenuVariantPresenter();
	const menuVariantController =
		overrides.menuVariantController ??
