...
} from "../../Applications/PaymentMethods/UseCases/index.js";
...
export default function registerPaymentMethodContainer({ container, overrides = {}, prisma }) {
	...
	const paymentMethodService =
		overrides.paymentMethodService ?? new PaymentMethodService({ paymentMethodRepository });
	const listPaymentMethodsUsecase =
		overrides.listPaymentMethodsUsecase ?? new ListPaymentMethodsUsecase({ paymentMethodService });
	const getPaymentMethodUsecase =
		overrides.getPaymentMethodUsecase ?? new GetPaymentMethodUsecase({ paymentMethodService });
	const createPaymentMethodUsecase =
		overrides.createPaymentMethodUsecase ?? new CreatePaymentMethodUsecase({ paymentMethodService });
	const updatePaymentMethodUsecase =
		overrides.updatePaymentMethodUsecase ?? new UpdatePaymentMethodUsecase({ paymentMethodService });
	const deletePaymentMethodUsecase =
		overrides.deletePaymentMethodUsecase ?? new DeletePaymentMethodUsecase({ paymentMethodService });
	const paymentMethodPresenter = overrides.paymentMethodPresenter ?? new PaymentMethodPresenter();
	const paymentMethodController =
		overrides.paymentMethodController ??
