...
export default function registerPlaceStockContainer({ container, overrides = {}, prisma }) {
	...
	const listPlaceStocksUsecase =
		overrides.listPlaceStocksUsecase ?? new ListPlaceStocksUsecase({ placeStockService });
	const getPlaceStockUsecase = overrides.getPlaceStockUsecase ?? new GetPlaceStockUsecase({ placeStockService });
	const createPlaceStockUsecase =
		overrides.createPlaceStockUsecase ?? new CreatePlaceStockUsecase({ placeStockService });
	const updatePlaceStockUsecase =
		overrides.updatePlaceStockUsecase ?? new UpdatePlaceStockUsecase({ placeStockService });
	const deletePlaceStockUsecase =
		overrides.deletePlaceStockUsecase ?? new DeletePlaceStockUsecase({ placeStockService });
	const placeStockPresenter = overrides.placeStockPresenter ?? new PlaceStockPresenter();
	const placeStockController =
		overrides.placeStockController ??
		new PlaceStockController({
			placeStockPresenter,
			listPlaceStocksUsecase,
			getPlaceStockUsecase,
			createPlaceStockUsecase,
