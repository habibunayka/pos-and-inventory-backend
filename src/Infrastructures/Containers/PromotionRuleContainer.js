...
} from "../../Applications/Promotions/UseCases/index.js";
...
export default function registerPromotionRuleContainer({ container, overrides = {}, prisma }) {
	...
	const promotionRuleService =
		overrides.promotionRuleService ?? new PromotionRuleService({ promotionRuleRepository });
	...
	const createUsecase =
		overrides.createPromotionRuleUsecase ?? new CreatePromotionRuleUsecase({ promotionRuleService });
	const deleteUsecase =
		overrides.deletePromotionRuleUsecase ?? new DeletePromotionRuleUsecase({ promotionRuleService });
	const presenter = overrides.promotionRulePresenter ?? new PromotionRulePresenter();
	const controller =
		overrides.promotionRuleController ??
		new PromotionRuleController({ presenter, listUsecase, getUsecase, createUsecase, deleteUsecase });
