...
export default function registerStockTransferContainer({ container, overrides = {}, prisma }) {
	...
	const stockTransferService =
		overrides.stockTransferService ?? new StockTransferService({ stockTransferRepository });
	...
	const createUsecase =
		overrides.createStockTransferUsecase ?? new CreateStockTransferUsecase({ stockTransferService });
	const deleteUsecase =
		overrides.deleteStockTransferUsecase ?? new DeleteStockTransferUsecase({ stockTransferService });
	const presenter = overrides.stockTransferPresenter ?? new StockTransferPresenter();
	const controller =
		overrides.stockTransferController ??
		new StockTransferController({ presenter, listUsecase, getUsecase, createUsecase, deleteUsecase });
