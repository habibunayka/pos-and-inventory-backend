...
} from "../../Applications/MenuVariantItems/UseCases/index.js";
...
export default function registerMenuVariantItemContainer({ container, overrides = {}, prisma }) {
	const menuVariantItemRepository =
		overrides.menuVariantItemRepository ?? new PrismaMenuVariantItemRepository({ prisma });
	const menuVariantItemService =
		overrides.menuVariantItemService ?? new MenuVariantItemService({ menuVariantItemRepository });
	const listMenuVariantItemsUsecase =
		overrides.listMenuVariantItemsUsecase ?? new ListMenuVariantItemsUsecase({ menuVariantItemService });
	const getMenuVariantItemUsecase =
		overrides.getMenuVariantItemUsecase ?? new GetMenuVariantItemUsecase({ menuVariantItemService });
	const createMenuVariantItemUsecase =
		overrides.createMenuVariantItemUsecase ?? new CreateMenuVariantItemUsecase({ menuVariantItemService });
	const updateMenuVariantItemUsecase =
		overrides.updateMenuVariantItemUsecase ?? new UpdateMenuVariantItemUsecase({ menuVariantItemService });
	const deleteMenuVariantItemUsecase =
		overrides.deleteMenuVariantItemUsecase ?? new DeleteMenuVariantItemUsecase({ menuVariantItemService });
	const menuVariantItemPresenter = overrides.menuVariantItemPresenter ?? new MenuVariantItemPresenter();
	const menuVariantItemController =
		overrides.menuVariantItemController ??
		new MenuVariantItemController({
