...
export default class GetPlaceStockUsecase {
	constructor({ placeStockService } = {}) {
		if (!placeStockService) throw new Error("GET_PLACESTOCK.MISSING_SERVICE");
	...
	async execute(id) {
		...
		const rec = await this.placeStockService.getPlaceStock(intId);
		if (!rec) throw new ValidationError("Place stock not found");
		return rec;
	}
}
