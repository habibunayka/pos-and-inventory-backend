...
export default class DeletePlaceStockUsecase {
	constructor({ placeStockService } = {}) {
		if (!placeStockService) throw new Error("DELETE_PLACESTOCK.MISSING_SERVICE");
	...
	async execute(id) {
		...
		const ok = await this.placeStockService.deletePlaceStock(intId);
		if (!ok) throw new ValidationError("Place stock not found");
		return true;
	}
}
