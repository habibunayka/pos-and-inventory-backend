...
export default class GetInventoryStockDailyUsecase {
	constructor({ inventoryStockDailyService } = {}) {
		if (!inventoryStockDailyService) throw new Error("GET_ISD.MISSING_SERVICE");
	...
	async execute(id) {
		...
		const rec = await this.inventoryStockDailyService.get(intId);
		if (!rec) throw new ValidationError("Inventory stock daily not found");
		return rec;
	}
}
