...
export default class DeleteInventoryStockDailyUsecase {
	constructor({ inventoryStockDailyService } = {}) {
		if (!inventoryStockDailyService) throw new Error("DELETE_ISD.MISSING_SERVICE");
	...
	async execute(id) {
		...
		const ok = await this.inventoryStockDailyService.delete(intId);
		if (!ok) throw new ValidationError("Inventory stock daily not found");
		return true;
	}
}
