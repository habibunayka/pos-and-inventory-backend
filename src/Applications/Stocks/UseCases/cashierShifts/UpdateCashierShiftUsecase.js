...
export default class UpdateCashierShiftUsecase {
	constructor({ cashierShiftService } = {}) {
		if (!cashierShiftService) throw new Error("UPDATE_CASHIER_SHIFT.MISSING_SERVICE");
	...
	async execute(id, payload = {}) {
		...
		if (payload.closedAt !== undefined)
			data.closedAt = payload.closedAt == null ? null : new Date(payload.closedAt);
		if (payload.closingBalance !== undefined)
			data.closingBalance = payload.closingBalance == null ? null : Number(payload.closingBalance);
		if (payload.systemBalance !== undefined)
			data.systemBalance = payload.systemBalance == null ? null : Number(payload.systemBalance);
