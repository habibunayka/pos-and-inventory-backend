...
export default class DeliveryIntegrationController {
	constructor({
		...
		if (miss) throw new Error(`DeliveryIntegrationController requires ${miss[0]}`);
	...
	async listDeliveryIntegrations() {
		const recs = await this.listDeliveryIntegrationsUsecase.execute();
		return { status: HttpStatus.OK, data: this.deliveryIntegrationPresenter.presentCollection(recs) };
	}
	async getDeliveryIntegration({ params }) {
		const rec = await this.getDeliveryIntegrationUsecase.execute(params.id);
		return { status: HttpStatus.OK, data: this.deliveryIntegrationPresenter.present(rec) };
	}
	async createDeliveryIntegration({ body }) {
		const rec = await this.createDeliveryIntegrationUsecase.execute(body);
		return { status: HttpStatus.CREATED, data: this.deliveryIntegrationPresenter.present(rec) };
	}
	async updateDeliveryIntegration({ params, body }) {
		const rec = await this.updateDeliveryIntegrationUsecase.execute(params.id, body);
		return { status: HttpStatus.OK, data: this.deliveryIntegrationPresenter.present(rec) };
	}
	async deleteDeliveryIntegration({ params }) {
		await this.deleteDeliveryIntegrationUsecase.execute(params.id);
